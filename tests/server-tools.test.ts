...
describe('MCP Server Tools', () => {
  ...
  describe('detectHallucinations functionality', () => {
    ...
    it('should detect hallucinated imports', () => {
      ...
      const hallucinatedImports = imports.filter(imp => 
    ...
    it('should validate against available libraries context', () => {
      ...
      const unknownImports = testImports.filter(imp => 
        !context.availableLibraries.some(lib => 
  ...
  describe('validateCodeQuality functionality', () => {
    it('should detect naming convention issues', () => {
      ...
      const projectUsesCamelCase = existingNames.filter(name => 
        /^[a-z][a-zA-Z0-9]*$/.test(name)
      ).length;
      
      const projectUsesSnakeCase = existingNames.filter(name => 
    ...
    it('should detect anti-patterns', () => {
      ...
      const detectedIssues = antiPatterns.filter(antiPattern => 
        code.match(antiPattern.pattern)
      );
    ...
    it('should detect security issues', () => {
      ...
      const securityIssues = securityPatterns.filter(pattern => 
