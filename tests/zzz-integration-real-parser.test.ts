...
describe('Real Parser Integration Tests', () => {
  ...
  describe('Complete indexing and validation workflow', () => {
    it('should index a file and then validate code against it', async () => {
      ...
      const parsedOriginal = parseCode(originalCode);
      ...
      const parsedNew = parseCode(newCode);
    ...
    it('should detect hallucinations in a complete workflow', async () => {
      ...
      const parsedExisting = parseCode(existingCode);
      ...
      const unknownImports = imports.filter(imp => 
        !availableLibraries.some(lib => 
          imp.includes(lib) || lib.includes(imp)
        )
      );
      ...
      const hallucinatedImports = unknownImports.filter(imp => 
        commonHallucinations.some(lib => imp.includes(lib))
      );

      expect(hallucinatedImports).toEqual(['ai-helper']);

      // Parse the suspicious code
      const parsedSuspicious = parseCode(suspiciousCode);
      ...
      const suspiciousFunctions = parsedSuspicious.functions.filter(func => 
    ...
    it('should provide quality analysis for a complete codebase', async () => {
      ...
      const snakeCaseCount = functionNames.filter(name => 
    ...
    it('should suggest improvements based on codebase patterns', async () => {
      ...
      const asyncFunctions = parsedExisting.functions.filter(f => 
        f.body.includes('await') || f.body.includes('async')
      );
      
      const functionsWithErrorHandling = parsedExisting.functions.filter(f => 
        f.body.includes('try') && f.body.includes('catch')
      );
  ...
  describe('End-to-end MCP tool simulation', () => {
    it('should simulate the complete MCP workflow', async () => {
      ...
      const indexResult = parseCode(fileContent);
      ...
      const validateResult = parseCode(codeToValidate);
      ...
      const hallucinationResult = parseCode(suspiciousCode);
      ...
      const qualityResult = parseCode(qualityTestCode);
